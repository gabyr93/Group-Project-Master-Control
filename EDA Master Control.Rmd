...
The goal of EDA is to: - Understand the structure and quality of the data\
...
Team members include **Corinn, Josh, Joel, and Gaby**, meeting weekly with the following milestones: - Feb 1 — Business problem statement finalized\
- Feb 22 — EDA complete\
...
Before we do any in depth cleaning and standardization we start by checking to see the number of unique and missing values for contact_lead_title field. This field is free form entry so has a huge variation, which in future modeling will create additional noise and difficulty. 
```{r}
...
```
Pre processing steps are completed to reduce the number of unique values. This includes removing punctuation or special characters, numbers,errant spacing, lowering the case and removing stop words
```{r}
...
To try and further reduce the number of unique values AI was leveraged to come up with a map of common abbreviations and typos. This value map was then applied to the data, replacing the values where they occured. 
```{r}
title_map <- c(
...
This further reduces the unique values down by another ~100 values. The problem now becomes that their are varrying combinations of common words that can occur in different frequencies. 
```{r}
...
```
If we look at a word cloud of the most common 50 words, their is no surprise at what shows up the most frequently. 
...
Below shows the top 10 occurring words, as well 10 that only occur 5 times, and for both of these  In total 361 words clear this threshold. 
...
Additionally we can look at just bi-grams (sequences of only two words from the title field), if we look at this it does give us some more information in regards to word order. But the number of unique word combinations then goes back up greatly, so this will not be further evaluated. 
```{r}
